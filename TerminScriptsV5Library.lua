...
local function SerializeValue(V)
    ...
    if type(V) == "table" then
        local T = {}
        for I, Item in ipairs(V) do T[I] = SerializeValue(Item) end
        return T
    end
    return V
end
...
function TerminScriptsLib.new(Title, Config)
    ...
    Self.ConfigFolderName = "TerminConfigs"
    Self:CreateMainGui()
    Self:SetupControls()
    Self:SetupDragging()
    Self:SetupKeybinds()
...
function TerminScriptsLib:CreateHeader()
    ...
    VersionLabel.Text = "V4 - Hood Games"
...
function TerminScriptsLib:TSToggle(Section, Config)
    ...
    local function UpdateToggle()
        ...
        if Section.Tab.Gui.AutoSaveEnabled and Section.Tab.Gui.CurrentConfigName ~= "" then
...
function TerminScriptsLib:TSKeyBind(Section, Config)
    ...
    local function GetKeybind() return CurrentKeybind end
    ...
    self:RegisterElement(Config.Key, GetKeybind, SetKeybind)
...
function TerminScriptsLib:TSSlider(Section, Config)
    ...
    self:RegisterElement(Key,
        ...
        function(V) CurrentValue = math.clamp(tonumber(V) or Min, Min, Max) UpdateSlider() end
    )
...
function TerminScriptsLib:TSMultiButton(Section, Config)
    ...
    self:RegisterElement(Config.Key, function() return Selected end, function(V) SelectOption(V) end)
...
function TerminScriptsLib:RegisterElement(Key, GetFn, SetFn)
    if not Key then
    ...
    for I, E in ipairs(self.ConfigRegistry) do
        if E.Key == Key then
            self.ConfigRegistry[I] = {Key = Key, Get = GetFn, Set = SetFn}
            return
        end
    end
    table.insert(self.ConfigRegistry, {Key = Key, Get = GetFn, Set = SetFn})
end

function TerminScriptsLib:SaveConfig(Name)
    Name = Name or self.CurrentConfigName
...
function TerminScriptsLib:ImportConfigFromClipboard()
    local Ok1, Content = pcall(function() return getclipboard() end)
    if not Ok1 or not Content then return false end
    local Ok2, Data = pcall(function() return HttpService:JSONDecode(Content) end)
    if not Ok2 or type(Data) ~= "table" then return false end
    for _, E in ipairs(self.ConfigRegistry) do
...
function TerminScriptsLib:CreateConfigTab()
    ...
    ActiveSection:TSButton({
        ...
        Style = "ghost",
        Callback = function()
            local Ok = self:ImportConfigFromClipboard()
            if Ok then
                self:Notify("Imported", "Config applied from clipboard.", "success", 2.5)
            else
                self:Notify("Import Failed", "Clipboard data is not valid config JSON.", "error", 3)
            end
        end,
    })
    ...
    local SelectedConfigName = ConfigListOptions[1]
    ...
    ManageSection:TSButton({
        ...
        Callback = function()
            ...
            local Success = self:LoadConfig(SelectedConfigName)
            if Success then
                self:Notify("Loaded", SelectedConfigName, "success", 2.5)
                RefreshActiveLabel()
            else
                self:Notify("Load Failed", "Could not read: " .. SelectedConfigName, "error", 3)
            end
        end,
    })
...
local function VerifyKey(KeyValue, Hwid)
    local Ok, Response = KeySystemRequest({
        ...
        Body = HttpService:JSONEncode({action = "verify-key", key = KeyValue, hwid = Hwid}),
    })
    ...
    if not DecodeOk or type(Data) ~= "table" then
        return false, nil, "Invalid response from license server"
    end
    if Data.valid then
        return true, Data, nil
    end
    return false, Data, Data.message or "Key verification failed"
end

local function CreateKeySystemGui()
    local Scale = ScalingManager.CurrentScale
    local KeyScreenGui = Instance.new("ScreenGui")
    KeyScreenGui.Name = "TerminKeySystem"
    ...
    local Width, Height = 460 * Scale, 400 * Scale
    local Card = Instance.new("Frame")
    Card.Name = "KeyCard"
    Card.Size = UDim2.new(0, Width, 0, Height)
    Card.Position = UDim2.new(0.5, -Width / 2, 0.5, -Height / 2)
    Card.BackgroundColor3 = Theme.Surface
    Card.BackgroundTransparency = Theme.GlassMain
    Card.BorderSizePixel = 0
    Card.ClipsDescendants = true
    Card.Parent = Overlay
    AddCorner(Card, 20 * Scale)
    local CardStroke = AddStroke(Card, 1.5 * Scale, Theme.BorderGlow, 0.3)
    CreateTween(CardStroke, Animations.GlowPulse, {Transparency = 0.05}):Play()
...
--[[
    local IconFrame = Instance.new("Frame")
    IconFrame.Size = UDim2.new(0, 56 * Scale, 0, 56 * Scale)
    IconFrame.Position = UDim2.new(0.5, -28 * Scale, 0, 32 * Scale)
    IconFrame.BackgroundColor3 = Theme.Primary
    IconFrame.BackgroundTransparency = 0.85
    IconFrame.BorderSizePixel = 0
    IconFrame.Parent = Card
    AddCorner(IconFrame, 16 * Scale)
    local IconStroke = AddStroke(IconFrame, 1.5 * Scale, Theme.Primary, 0.35)
    ...
    IconLabel.Text = " "
    IconLabel.TextColor3 = Theme.Primary
    IconLabel.TextSize = 26 * Scale
    IconLabel.Font = Enum.Font.GothamBlack
    IconLabel.Parent = IconFrame]]

    local TitleLabel = Instance.new("TextLabel")
    TitleLabel.Size = UDim2.new(1, -40 * Scale, 0, 30 * Scale)
    TitleLabel.Position = UDim2.new(0, 20 * Scale, 0, 100 * Scale)
    TitleLabel.BackgroundTransparency = 1
    TitleLabel.Text = "Termin Scripts Key System"
    TitleLabel.TextColor3 = Theme.TextPrimary
    TitleLabel.TextSize = 19 * Scale
    TitleLabel.Font = Enum.Font.GothamBold
    TitleLabel.Parent = Card

    local SubtitleLabel = Instance.new("TextLabel")
    SubtitleLabel.Size = UDim2.new(1, -60 * Scale, 0, 40 * Scale)
    SubtitleLabel.Position = UDim2.new(0, 30 * Scale, 0, 132 * Scale)
    SubtitleLabel.BackgroundTransparency = 1
    SubtitleLabel.Text = "Enter your license key to unlock the script"
    SubtitleLabel.TextColor3 = Theme.TextMuted
    SubtitleLabel.TextSize = 13 * Scale
    SubtitleLabel.TextWrapped = true
    SubtitleLabel.Font = Enum.Font.Gotham
    SubtitleLabel.Parent = Card

    local InputFrame = Instance.new("Frame")
    InputFrame.Size = UDim2.new(1, -60 * Scale, 0, 48 * Scale)
    InputFrame.Position = UDim2.new(0, 30 * Scale, 0, 190 * Scale)
    InputFrame.BackgroundColor3 = Theme.Card
    InputFrame.BackgroundTransparency = 0.15
    InputFrame.BorderSizePixel = 0
    InputFrame.Parent = Card
    AddCorner(InputFrame, 12 * Scale)
    local InputStroke = AddStroke(InputFrame, 1.5 * Scale, Theme.Primary, 0.5)

    local KeyBox = Instance.new("TextBox")
    KeyBox.Size = UDim2.new(1, -28 * Scale, 1, 0)
    KeyBox.Position = UDim2.new(0, 14 * Scale, 0, 0)
    KeyBox.BackgroundTransparency = 1
    KeyBox.Text = ""
    KeyBox.PlaceholderText = "TERMIN-XXXX-XXXX-XXXX"
    ...
    KeyBox.Focused:Connect(function()
        CreateTween(InputStroke, Animations.Fast, {Color = Theme.PrimaryGlow, Transparency = 0.15}):Play()
    end)
    KeyBox.FocusLost:Connect(function()
        CreateTween(InputStroke, Animations.Fast, {Color = Theme.Primary, Transparency = 0.5}):Play()
    end)

    local StatusLabel = Instance.new("TextLabel")
    StatusLabel.Size = UDim2.new(1, -60 * Scale, 0, 24 * Scale)
    StatusLabel.Position = UDim2.new(0, 30 * Scale, 0, 244 * Scale)
    ...
    SubmitBtn.Size = UDim2.new(1, -60 * Scale, 0, 46 * Scale)
    SubmitBtn.Position = UDim2.new(0, 30 * Scale, 0, 276 * Scale)
    ...
    AddCorner(SubmitBtn, 12 * Scale)
    local SubmitStroke = AddStroke(SubmitBtn, 1 * Scale, Theme.PrimaryGlow, 0.5)

    local SubmitLabel = Instance.new("TextLabel")
    SubmitLabel.Size = UDim2.new(1, 0, 1, 0)
    SubmitLabel.BackgroundTransparency = 1
    SubmitLabel.Text = "Authenticate"
    SubmitLabel.TextColor3 = Color3.fromRGB(10, 15, 12)
    SubmitLabel.TextSize = 15 * Scale
    SubmitLabel.Font = Enum.Font.GothamBold
    SubmitLabel.Parent = SubmitBtn

    SubmitBtn.MouseEnter:Connect(function()
        CreateTween(SubmitBtn, Animations.Fast, {BackgroundColor3 = Theme.PrimaryHover}):Play()
    end)
    SubmitBtn.MouseLeave:Connect(function()
        CreateTween(SubmitBtn, Animations.Fast, {BackgroundColor3 = Theme.Primary}):Play()
    end)

    local FooterLabel = Instance.new("TextLabel")
    FooterLabel.Size = UDim2.new(1, -40 * Scale, 0, 20 * Scale)
    FooterLabel.Position = UDim2.new(0, 20 * Scale, 1, -34 * Scale)
    FooterLabel.BackgroundTransparency = 1
    FooterLabel.Text = "Don't have a key? Purchase access on the TerminScripts site"
    FooterLabel.TextColor3 = Theme.TextDisabled
    FooterLabel.TextSize = 11 * Scale
    FooterLabel.TextWrapped = true
    FooterLabel.Font = Enum.Font.Gotham
    FooterLabel.Parent = Card

    local HwidButton = Instance.new("TextButton")
    HwidButton.Size = UDim2.new(0, 148 * Scale, 0, 22 * Scale)
    HwidButton.Position = UDim2.new(0, 30 * Scale, 1, -60 * Scale)
    HwidButton.BackgroundColor3 = Theme.Card
    HwidButton.BackgroundTransparency = 0.3
    HwidButton.Text = "⊕  Copy my device ID"
    HwidButton.TextColor3 = Theme.TextMuted
    HwidButton.TextSize = 11 * Scale
    HwidButton.Font = Enum.Font.GothamMedium
    HwidButton.AutoButtonColor = false
    HwidButton.Parent = Card
    AddCorner(HwidButton, 6 * Scale)
    AddStroke(HwidButton, 1 * Scale, Theme.Border, 0.6)

    HwidButton.MouseEnter:Connect(function()
        CreateTween(HwidButton, Animations.Fast, {BackgroundTransparency = 0.1, TextColor3 = Theme.TextSecondary}):Play()
    end)
    HwidButton.MouseLeave:Connect(function()
        CreateTween(HwidButton, Animations.Fast, {BackgroundTransparency = 0.3, TextColor3 = Theme.TextMuted}):Play()
    end)

    HwidButton.MouseButton1Click:Connect(function()
        local DeviceId = GetHwid()
        print("[TerminKeySystem] Device ID: " .. DeviceId)
        local SetClipboardFn = (typeof(setclipboard) == "function" and setclipboard)
            or (typeof(toclipboard) == "function" and toclipboard)
            or nil
        if SetClipboardFn then
            pcall(SetClipboardFn, DeviceId)
            HwidButton.Text = "✓  Copied!"
            CreateTween(HwidButton, Animations.Fast, {TextColor3 = Theme.Success}):Play()
        else
            HwidButton.Text = "⊕  Printed to console"
        end
        task.delay(2.5, function()
            if HwidButton and HwidButton.Parent then
                HwidButton.Text = "⊕  Copy my device ID"
                CreateTween(HwidButton, Animations.Fast, {TextColor3 = Theme.TextMuted}):Play()
            end
        end)
    end)

    Card.Size = UDim2.new(0, Width * 0.9, 0, Height * 0.9)
    Card.BackgroundTransparency = 1
    task.delay(0.05, function()
        CreateTween(Card, Animations.Spring, {
    ...
    function Handle:SetLoading(IsLoading)
        KeyBox.TextEditable = not IsLoading
        SubmitBtn.Active = not IsLoading
        SubmitLabel.Text = IsLoading and "Verifying..." or "Authenticate"
        CreateTween(SubmitBtn, Animations.Fast, {BackgroundTransparency = IsLoading and 0.4 or 0.05}):Play()
    end
    ...
    function Handle:OnSubmit(Callback)
        local function Fire()
            CreateRipple(SubmitBtn, Vector2.new(SubmitBtn.AbsoluteSize.X / 2, SubmitBtn.AbsoluteSize.Y / 2))
            Callback(KeyBox.Text)
        end
        SubmitBtn.MouseButton1Click:Connect(Fire)
    ...
    function Handle:Destroy()
        CreateTween(Card, Animations.FadeOut, {BackgroundTransparency = 1}):Play()
        CreateTween(Overlay, Animations.FadeOut, {BackgroundTransparency = 1}):Play()
        task.delay(0.25, function()
            if KeyScreenGui and KeyScreenGui.Parent then KeyScreenGui:Destroy() end
        end)
    end
...
local function CreateLoaderGui()
    ...
    LoaderScreenGui.Name = "TerminKeySystemLoader"
    ...
    LoaderScreenGui.DisplayOrder = 998
    ...
    Overlay.BorderSizePixel = 0
    Overlay.Parent = LoaderScreenGui

    local AmbientGlow = Instance.new("ImageLabel")
    AmbientGlow.Name = "AmbientGlow"
    AmbientGlow.Size = UDim2.new(0, 400 * Scale, 0, 400 * Scale)
    AmbientGlow.Position = UDim2.new(0.5, -200 * Scale, 0.5, -200 * Scale)
    AmbientGlow.BackgroundTransparency = 1
    AmbientGlow.Image = "rbxassetid://10471322156"
    AmbientGlow.ImageColor3 = Theme.Primary
    AmbientGlow.ImageTransparency = 0.85
    AmbientGlow.Parent = Overlay

    local OuterRing = Instance.new("ImageLabel")
    OuterRing.Size = UDim2.new(0, 120 * Scale, 0, 120 * Scale)
    OuterRing.Position = UDim2.new(0.5, -60 * Scale, 0.5, -135 * Scale)
    OuterRing.BackgroundTransparency = 1
    OuterRing.Image = "rbxassetid://10014761427" 
    OuterRing.ImageColor3 = Theme.Secondary
    OuterRing.ImageTransparency = 0.3
    OuterRing.Parent = Overlay

    local Ring = Instance.new("ImageLabel")
    Ring.Size = UDim2.new(0, 100 * Scale, 0, 100 * Scale)
    Ring.Position = UDim2.new(0.5, -50 * Scale, 0.5, -125 * Scale)
    Ring.BackgroundTransparency = 1
    Ring.Image = "rbxassetid://10014761427" 
    Ring.ImageColor3 = Theme.Primary
    Ring.ImageTransparency = 0.1
    Ring.Parent = Overlay

    task.spawn(function()
        while Ring and Ring.Parent do
            local T = CreateTween(Ring, TweenInfo.new(1.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.InOut), {Rotation = 360})
            local T2 = CreateTween(OuterRing, TweenInfo.new(3, Enum.EasingStyle.Linear), {Rotation = -360})
            T:Play()
            T2:Play()
            T.Completed:Wait()
            Ring.Rotation = 0
    ...
    local TitleLabel = Instance.new("TextLabel")
    TitleLabel.Size = UDim2.new(0, 350 * Scale, 0, 30 * Scale)
    TitleLabel.Position = UDim2.new(0.5, -175 * Scale, 0.5, -35 * Scale)
    TitleLabel.BackgroundTransparency = 1
    TitleLabel.Text = "TERMIN SCRIPTS"
    TitleLabel.TextColor3 = Theme.TextPrimary
    ...
    local StatusLabel = Instance.new("TextLabel")
    StatusLabel.Size = UDim2.new(0, 350 * Scale, 0, 20 * Scale)
    StatusLabel.Position = UDim2.new(0.5, -175 * Scale, 0.5, 2 * Scale)
    StatusLabel.BackgroundTransparency = 1
    StatusLabel.Text = "Initializing..."
    StatusLabel.TextColor3 = Theme.TextMuted
    StatusLabel.TextSize = 13 * Scale
    StatusLabel.Font = Enum.Font.GothamMedium
    StatusLabel.Parent = Overlay

    local TrackWidth = 280 * Scale
    local Track = Instance.new("Frame")
    Track.Size = UDim2.new(0, TrackWidth, 0, 6 * Scale)
    Track.Position = UDim2.new(0.5, -TrackWidth / 2, 0.5, 38 * Scale)
    Track.BackgroundColor3 = Theme.Card
    Track.BorderSizePixel = 0
    ...
    local Shimmer = Instance.new("Frame")
    Shimmer.Size = UDim2.new(0, 40 * Scale, 1, 0)
    Shimmer.Position = UDim2.new(0, -40 * Scale, 0, 0)
    Shimmer.BackgroundColor3 = Color3.new(1, 1, 1)
    ...
    task.spawn(function()
        while Shimmer and Shimmer.Parent do
            CreateTween(Shimmer, TweenInfo.new(1.2, Enum.EasingStyle.Sine), {Position = UDim2.new(1, 0, 0, 0)}):Play()
            task.wait(1.5)
            Shimmer.Position = UDim2.new(0, -40 * Scale, 0, 0)
        end
    end)
    ...
    local Handle = {}
    Handle.ScreenGui = LoaderScreenGui

    function Handle:Run(OnComplete)
        local Phases = {
            {Text = "Connecting to Secure Server...", Progress = 0.35},
            {Text = "Injecting Optimized Assets...", Progress = 0.75},
            {Text = "Finalizing...", Progress = 1.0},
        }
        
        for _, Phase in ipairs(Phases) do
            StatusLabel.Text = Phase.Text
            CreateTween(Fill, TweenInfo.new(0.7, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
        ...
        local TweenInfoConfig = TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        
        CreateTween(Overlay, TweenInfoConfig, {BackgroundTransparency = 1}):Play()
        CreateTween(Ring, TweenInfoConfig, {ImageTransparency = 1}):Play()
        CreateTween(OuterRing, TweenInfoConfig, {ImageTransparency = 1}):Play()
        CreateTween(AmbientGlow, TweenInfoConfig, {ImageTransparency = 1}):Play()
        CreateTween(TitleLabel, TweenInfoConfig, {TextTransparency = 1}):Play()
        CreateTween(StatusLabel, TweenInfoConfig, {TextTransparency = 1}):Play()
        CreateTween(Track, TweenInfoConfig, {BackgroundTransparency = 1}):Play()
        CreateTween(Fill, TweenInfoConfig, {BackgroundTransparency = 1}):Play()
