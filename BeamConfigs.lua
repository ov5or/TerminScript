return {
    ["8-Bit"] = {
        ...
        animate = function(Beam)
            local FadeTween = TweenInfo.new(1)
            TweenService:Create(Beam, FadeTween, {Width1 = 0}):Play()
            FadeTween = TweenInfo.new(0.8)
            TweenService:Create(Beam, FadeTween, {Width0 = 0}):Play()
        end,
        debris = 1
    },
    ...
    ["Cosmic"] = {
        ...
        animate = function(Beam)
            local FadeTween = TweenInfo.new(0.3, Enum.EasingStyle.Sine)
            TweenService:Create(Beam, FadeTween, {Width0 = 0.08, Width1 = 0.05}):Play()
            task.spawn(function()
                while Beam.Parent do
                    local FirstTween = TweenInfo.new(1.5, Enum.EasingStyle.Sine)
                    TweenService:Create(Beam, FirstTween, {TextureSpeed = 2}):Play()
                    task.wait(1.5)
                    if not Beam.Parent then
                        break
                    end
                    local SecondTween = TweenInfo.new(1.5, Enum.EasingStyle.Sine)
                    TweenService:Create(Beam, SecondTween, {TextureSpeed = 0.5}):Play()
                    task.wait(1.5)
                end
            end)
            task.delay(1.5, function()
                if Beam and Beam.Parent then
                    local FinalTween = TweenInfo.new(0.5, Enum.EasingStyle.Sine)
                    TweenService:Create(Beam, FinalTween, {Width0 = 0, Width1 = 0}):Play()
                end
            end)
        end,
        debris = 1.5
    ...
    ["Electric"] = {
        ...
        animate = function(Beam)
            local FadeTween = TweenInfo.new(0.3)
            TweenService:Create(Beam, FadeTween, {Width0 = 0.5, Width1 = 0.5}):Play()
            task.spawn(function()
                while Beam.Parent do
                    local FirstTween = TweenInfo.new(0.8, Enum.EasingStyle.Sine)
                    TweenService:Create(Beam, FirstTween, {TextureSpeed = 10}):Play()
                    task.wait(0.8)
                    if not Beam.Parent then
                        break
                    end
                    local SecondTween = TweenInfo.new(0.8, Enum.EasingStyle.Sine)
                    TweenService:Create(Beam, SecondTween, {TextureSpeed = 2}):Play()
                    task.wait(0.8)
                end
            end)
            task.delay(2.5, function()
                if Beam and Beam.Parent then
                    local FinalTween = TweenInfo.new(1)
                    TweenService:Create(Beam, FinalTween, {Width0 = 0, Width1 = 0}):Play()
                end
            end)
        end,
        ...
        animate = function(Beam)
            task.spawn(function()
                for i = 1, 8, 1 do
                    ...
                    local FadeTween = 0.15 + math.random() * 0.3
                    local PulseTween = TweenInfo.new(0.1)
                    TweenService:Create(Beam, PulseTween, {Width0 = FadeTween, Width1 = FadeTween * 1.5}):Play()
                    task.wait(0.1)
                end
            end)
            task.delay(0.8, function()
                if Beam and Beam.Parent then
                    local FadeTween = TweenInfo.new(0.6)
                    TweenService:Create(Beam, FadeTween, {Width0 = 0, Width1 = 0}):Play()
                end
            end)
        end,
        debris = 1
    ...
    ["Flash"] = {
        ...
        animate = function(Beam)
            local FadeTween = TweenInfo.new(0.1)
            TweenService:Create(Beam, FadeTween, {Width0 = 0.22, Width1 = 0.16}):Play()
            task.spawn(function()
                for i = 1, 5, 1 do
                    if not Beam.Parent then
                        break
                    end
                    local FirstTween = TweenInfo.new(0.08)
                    TweenService:Create(Beam, FirstTween, {
            ...
            task.delay(0.5, function()
                if Beam and Beam.Parent then
                    Beam.Transparency = NumberSequence.new(0.8, 1)
                    local SecondTween = TweenInfo.new(0.3)
                    TweenService:Create(Beam, SecondTween, {Width0 = 0, Width1 = 0}):Play()
                end
            end)
        end,
        debris = 0.8
    ...
    ["Frost"] = {
        ...
        animate = function(Beam)
            local FadeTween = TweenInfo.new(1)
            TweenService:Create(Beam, FadeTween, {Width1 = 0.09}):Play()
            FadeTween = TweenInfo.new(0.8)
            TweenService:Create(Beam, FadeTween, {Width0 = 0.09}):Play()
            task.delay(0.1, function()
                if not Beam.Parent then
                    return
                end
                local FirstTween = TweenInfo.new(1.2)
                TweenService:Create(Beam, FirstTween, {Width1 = 0}):Play()
                local SecondTween = TweenInfo.new(1.15)
                TweenService:Create(Beam, SecondTween, {Width0 = 0}):Play()
            end)
        end,
        debris = 2
    },
    ...
    ["Ice"] = {
        ...
        animate = function(Beam)
            local FadeTween = TweenInfo.new(0.4)
            TweenService:Create(Beam, FadeTween, {Width0 = 0.5, Width1 = 0.25}):Play()
            task.delay(0.4, function()
                if not Beam.Parent then
                    return
                end
                local FirstTween = TweenInfo.new(0.6)
                TweenService:Create(Beam, FirstTween, {Width0 = 0.1, Width1 = 0.05}):Play()
            end)
            task.delay(1.2, function()
                if not Beam.Parent then
                    return
                end
                local SecondTween = TweenInfo.new(0.8)
                TweenService:Create(Beam, SecondTween, {Width0 = 0, Width1 = 0}):Play()
            end)
        end,
        debris = 1.5
    ...
    ["Laser"] = {
        ...
        animate = function(Beam)
            local FadeTween = TweenInfo.new(0.2)
            TweenService:Create(Beam, FadeTween, {Width1 = 0.22, Width0 = 0.22}):Play()
            task.delay(0.2, function()
                if not Beam.Parent then
                    return
                end
                local FirstTween = TweenInfo.new(0.8)
                TweenService:Create(Beam, FirstTween, {Width1 = 0.06, Width0 = 0.06}):Play()
            end)
        end,
        debris = 1
    },
    ...
    ["Light"] = {
        ...
        animate = function(Beam)
            local FadeTween = TweenInfo.new(0.3)
            TweenService:Create(Beam, FadeTween, {Width0 = 0.4, Width1 = 0.4}):Play()
            task.spawn(function()
                while Beam.Parent do
                    local FirstTween = TweenInfo.new(0.4, Enum.EasingStyle.Sine)
                    TweenService:Create(Beam, FirstTween, {LightEmission = 1}):Play()
                    task.wait(0.4)
                    if not Beam.Parent then
                        break
                    end
                    local SecondTween = TweenInfo.new(0.4, Enum.EasingStyle.Sine)
                    TweenService:Create(Beam, SecondTween, {LightEmission = 0.6}):Play()
                    task.wait(0.4)
                end
            end)
            task.delay(1.5, function()
                if Beam and Beam.Parent then
                    local FinalTween = TweenInfo.new(0.8)
                    TweenService:Create(Beam, FinalTween, {Width0 = 0, Width1 = 0}):Play()
                end
            end)
        end,
    ...
    ["Linger"] = {
        ...
        animate = function(Beam)
            local FadeTween = TweenInfo.new(0.075)
            TweenService:Create(Beam, FadeTween, {Width1 = 0.09}):Play()
            FadeTween = TweenInfo.new(0.075)
            TweenService:Create(Beam, FadeTween, {Width0 = 0.09}):Play()
            task.delay(0.1, function()
                if not Beam.Parent then
                    return
